...
| v1.1 | 2026-02-25 | 1. 明确提醒功能核心地位与架构保障<br>2. 补充DeepSeek API动态配置架构<br>3. 更新风险评估与应对策略 | - |
...
> **提醒功能是产品的核心差异化竞争力，架构设计必须确保提醒功能的可靠性、及时性和可扩展性。**

**架构保障要求**:

| 保障维度 | 目标值 | 架构措施 |
|----------|--------|----------|
| 提醒触达率 | ≥99.5% | 独立提醒服务、消息队列、失败重试机制 |
| 提醒及时性 | 误差≤5分钟 | 分布式定时任务、优先级队列 |
| 服务可用性 | 99.9% | 服务隔离、熔断降级、监控告警 |
| 成本控制 | 动态可调 | API调用配额动态管理、优先级队列 |

**架构设计原则**:
1. **服务隔离**: 提醒服务独立部署，与主业务服务解耦
2. **优先级保障**: 重点院校信息优先处理，确保核心用户体验
3. **弹性伸缩**: API调用配额根据业务量动态调整，高峰期自动扩容
4. **全链路监控**: 从提醒创建到消息送达全链路可观测
...
| 平台约束 | 微信小程序平台规范限制 |
| 合规约束 | 《个人信息保护法》、微信平台规范 |
| 成本约束 | DeepSeek API调用成本可控，支持动态扩缩容 |
...
## 6. DeepSeek API 动态配置架构

### 6.1 配置设计原则

> **以每日接收免试研究生的院校所发布的夏令营公告数量为基准，设置尽可能高的API调用上限值。**

**基础数据参考**:
- 全国具有保研资格的院校: 366所
- 夏令营高峰期(4-7月)日均公告量: 50-300条
- 预推免期(8-10月)日均公告量: 30-150条
- 单条公告AI处理成本: ~0.1元
...
interface DeepSeekConfig {
  // 基础限制
  dailyBaseLimit: number;      // 日常基础配额: 200次/天
  dailyPeakLimit: number;      // 高峰期配额: 500次/天
  monthlyLimit: number;        // 月度上限: 12000次/月
  
  // 动态扩缩容策略
  dynamicScaling: {
    enabled: boolean;
    scaleUpThreshold: number;   // 使用率超80%触发扩容
    scaleUpFactor: number;      // 扩容系数1.5倍
    maxDailyLimit: number;      // 单日最大上限: 800次
    cooldownPeriod: number;     // 扩容冷却期: 24小时(毫秒)
  };
  ...
  // 成本控制配置
  costControl: {
    dailyBudget: number;        // 日预算上限: 50元
    monthlyBudget: number;      // 月预算上限: 1000元
    alertThreshold: number;     // 预算使用超80%触发告警
  };
}

interface PriorityTier {
  level: 'P0' | 'P1' | 'P2' | 'P3';
  universities: string[];       // 院校名称或标签
  maxAge: number;              // 最大等待时间(毫秒)
  weight: number;              // 处理权重
}
